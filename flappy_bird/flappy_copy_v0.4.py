...
while run:
	...
	print(abs(((flappy.rect.y - 50 ) // 4)-90))
	...
	if flying == True and game_over == False:
		...
		if time_now - last_pipe > pipe_frequency:
			...
			# if pipe_height >= pipeLowerLimit and flag == 0:
			# 	pipe_height += pipeHeightIncrement #	method to generate systematic pipes 
			# 	if pipe_height <= pipeLowerLimit:
			# 		flag = 1
			# elif pipe_height <= pipeUpperLimit and flag == 1:
			# 	pipe_height += pipeHeightIncrement
			# 	if pipe_height >= pipeUpperLimit:
			# 		flag = 0

			if pipe_height >= pipeLowerLimit and flag == 1:
				pipe_height -= pipeHeightIncrement #	method to generate systematic pipes 
				if pipe_height <= pipeLowerLimit or pipe_height <= -((screen_height - 100) / 2):
					pipe_height = pipeLowerLimit
					# pipe_height = int(max(pipeLowerLimit, -((screen_height - 200) / 2)))
					flag = 0
			elif pipe_height <= pipeUpperLimit and flag == 0:
				pipe_height += pipeHeightIncrement
				if pipe_height >= pipeUpperLimit or pipe_height >= (screen_height - 100) / 2:
					pipe_height = pipeUpperLimit
					# pipe_height = int(min(pipeUpperLimit, ((screen_height - 200) / 2)))
					flag = 1

			# if pipe_height == -1 * pipeHeightPiviot:
			# 	pipe_height = pipeHeightPiviot
			# 	flag = 1

			# elif pipe_height == pipeHeightPiviot:
			# 	pipe_height = -1 * pipeHeightPiviot
			# 	flag = 0
